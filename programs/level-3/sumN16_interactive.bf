# Sum N numbers (16 bit sum) (with ASCII conversion); interactive version
...
[
    ...
    ### on space or null byte or newline : sum the result and the current numbers
    ,>>++               # get new input digit; select and increment space flag twice (tmp@Space)
    <<[>>-<<<]<[>]      # if input is not zero decrement space flag

    ## check if input is newline; if so : increment space flag and reset input to null
    >----------             # subtract \n (10) from input (now it is null if it is newline)
    [++++++++++>>-<<<]<[>]  # if not null : restore input value and decrement space flag
...
>>>>>[ # Error occurred; print error message and offending character
    >           # deselect ErrorChar
    # print "Failed to compute sum: invalid character: "
    >++++++++++++++[>++++>++>++++++++>+++++++>+++++++>+++++[<]>-]>++>++++>++++>->+++
    ++++>.<<.>.+++.<++++.-.<<.>.-----.<.>>-.<.>>+.<<+.+++++.-.>++.<<.>-.++.>>.<<<<.>
    .>>>----.+++++.<<+.>----.>--.---.<+++.<<.>>-.>-.<--.<----.>.++.<++.>++.<--.<<.>.
    [>]<[[-]<]<
