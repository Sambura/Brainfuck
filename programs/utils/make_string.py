...
def values_to_text(values: list[int], escape=True):
    text = ''.join(map(chr, values))
    return text.replace('\n', '\\n').replace('\r', '\\r') if escape else text
...
def main():
    ...
    parser.add_argument('--escape-characters', default=None, action=argparse.BooleanOptionalAction, help='Interpret special characters such as \\n or \\u1234. Disabled by default for file or preset inputs')
    parser.add_argument('--playback', action='store_true', help='Print the output of the program')

    args = parser.parse_args()
    verbose = args.verbose # set global verbose level
    escape_characters_default = False

    if args.playback and args.no_verify:
        print('Cannot do playback when verification is disabled')
        exit(1)
    ...
    if input_methods > 1:
        ...
        input_data = input('Input a string you would like to encode in BF: ')
        escape_characters_default = True
        ...
        input_data = input_presets[args.preset]
    ...
    do_escape_characters = escape_characters_default if args.escape_characters is None else args.escape_characters
    if do_escape_characters:
        input_data = input_data.encode().decode('unicode_escape')

    input_data = text_to_value_list(input_data)
    ...
    for program, algo in programs:
        ...
        if not args.no_verify:
            ok, program_output = verify_program(program, input_data, return_details=True)
            if not ok:
        ...
        if args.playback:
            print(f'Program output: "{values_to_text(program_output, escape=False)}"')
